...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> dict[str, torch.Tensor]:
        """
        Returns:
            dict[str, torch.Tensor]:
                log_probs: (bs, response_len)
                entropys: (bs, response_len), optional when calculate_entropy=True
                advantage_preds: (bs, response_len), optional when predict_advantage_head=True during training
        """
    ...
    def _compute_advantage_padded(
        ...
        # Next-token alignment: h_{t-1} predicts realized action a_t.
            ...
            if self.predict_advantage_head:
                for mini_batch in mini_batches:
                    ...
                    for micro_batch in micro_batches:
                        ...
                        append_to_dict(
                            metrics,
                            {"actor/advantage_loss": advantage_loss.detach().item() * loss_scale_factor},
                        )
